...
from .idm_engine import IDMVtonEngine


settings = get_settings()
settings.idm_vton_output_dir.mkdir(parents=True, exist_ok=True)
settings.idm_vton_temp_dir.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="IDM-VTON Inference Service")
app.mount("/outputs", StaticFiles(directory=settings.idm_vton_output_dir), name="outputs")
engine = IDMVtonEngine(settings)
...
async def infer(
    ...
    try:
        ...
        result = engine.infer(person_path, garment_path, category, garment_description)
...
async def _persist_upload(upload: UploadFile, field_name: str) -> Path:
    ...
    max_bytes = settings.idm_vton_max_upload_mb * 1024 * 1024
    data = await upload.read()
    if len(data) == 0:
        raise TryOnError("INVALID_IMAGE", f"{field_name} is empty.", 400)
    if len(data) > max_bytes:
        raise TryOnError("INVALID_IMAGE", f"{field_name} exceeds {settings.idm_vton_max_upload_mb} MB.", 400)

    suffix = Path(upload.filename or "").suffix.lower()
    if suffix not in {".jpg", ".jpeg", ".png", ".webp"}:
        suffix = ".png"

    with NamedTemporaryFile(delete=False, suffix=suffix, dir=settings.idm_vton_temp_dir) as file:
        file.write(data)
